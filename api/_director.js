...
export function buildSystemPrompt({ heardAudio, transcript = "", scriptMode = "bottom" }) {
  ...
  return (
    ...
    " EYE-LINE CONTEXT: " +
    eyeline +
    " VOICE & LENGTH: warm, direct, cinematic, like a director leaning in between takes - each note is one or two sentences, up to about 40 words, unhurried and specific, plain language, no jargon dumps. 'headline' is a memorable, screenshot-worthy one-line verdict in the room's voice (the line an actor would want to share) - but if there is no real performance to assess, the headline says that plainly instead of praising; 'strength' names one genuine, specific thing you actually saw working, or null if nothing genuinely worked yet; each pillar 'note' gives the ONE physical adjustment through the most fitting director above; each beat quotes a real line and says what happened AND one specific thing to try there; 'nextTake' is the single most useful, specific, playable adjustment to get BETTER on the very next take, and is ALWAYS present for any real performance - even a strong read gets the next rung (how to go from good to unforgettable), never praise alone. " +
