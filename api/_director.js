...
export function audioContext(heardAudio, transcript = "") {
  if (heardAudio) {
    return {
      text:
        ...
        transcript +
        '". FIRST compare it to the SCRIPT. If they spoke real lines (even improvised or off-script), grade the performance normally and quote beats from the ACTUAL transcript. If what they said is NOT a performance - gibberish, humming, beat-boxing, counting, or unrelated sounds - then there is NO acting to grade: set "castable" to null, set every pillar score to null, set "beats" to [], and name plainly and kindly what you actually heard instead of inventing a scene. NEVER invent lines they did not say and NEVER assign a low number to a non-performance - use null.',
      pacingNull: false,
      dictionNull: false,
    };
  }
  return {
    text:
      "NO words were spoken and NO speech was detected this take - the actor said nothing. With no audio, the DEFAULT is that this is NOT a gradeable acting performance. Treat it as a real performance ONLY if the FRAMES show a clear, deliberately CHANGING emotional intention played across the take (a committed silent reaction). If the actor is instead just sitting, chewing, staring neutrally, glancing around, fidgeting, or testing the camera, there is NO performance to grade: set \"castable\" to null, set EVERY pillar score to null, set \"beats\" to [], set \"strength\" to null, and make the headline name it warmly and plainly - for example: \"It doesn't look like you gave me a performance to read this take - if you meant to act silently, pick one clear intention and let it move across your face; if you meant to speak, I didn't hear you.\" NEVER assign a low number to a non-performance - use null. If it genuinely IS a committed silent performance, assess facial emotion, eye-line, and presence and score honestly. Either way you CANNOT assess the voice: return null for pacing and diction with the note 'Not assessed - no audio detected.'",
    pacingNull: true,
    dictionNull: true,
  };
}
